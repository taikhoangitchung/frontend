...
export default function RoomRankingPanel({code}) {
    ...
    const showDetail = async (historyId) => {
        try {
            ...
            const mappedQuestions = detail.fullQuestions.map((q) => ({
                ...q,
                selectedAnswerIds:
                    detail.choices.find((c) => c.questionId === q.id)?.selectedAnswerIds || [],
            }))
    ...
    return (
        <div className="relative">
            <div
                className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-xl shadow-lg text-white space-y-4 w-full h-full">
